...
export async function executeInMemoryQuery(text, params = []) {
  ...
  if (/^INSERT INTO transactions/i.test(normalized)) {
    ...
    let account_id, amount, balance_after, reference, transfer_id = null, created_by = null, related_transaction_id = null;

    if (normalized.includes('related_transaction_id')) {
      // (account_id, type, amount, balance_after, reference, related_transaction_id, created_by)
      account_id = params[0];
      amount = BigInt(params[1]);
      balance_after = BigInt(params[2]);
      reference = params[3];
      related_transaction_id = params[4] || null;
      created_by = params[5] || null;
    } else if (normalized.includes('transfer_id')) {
...
export function serializeAcc(a) {
...
export function serializeLoan(l) {
